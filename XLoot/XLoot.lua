...
XLoot.revision  = tonumber((string.gsub("$Revision: 18443 $", "^%$Revision: (%d+) %$$", "%1")))
...
function XLoot:OnInitialize()
	...
	self:SetupFrames()
	self._scanTip = self._scanTip or CreateFrame("GameTooltip","XLootScanTooltip",nil,UIParent)
	...
	if XLoot.compat then
		...
		function XLoot:ItemInfo(num) -- itemName, itemString, itemQuality, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture
			...
			if (itemName) then
				...
				XLoot._scanTip = XLoot._scanTip or CreateFrame("GameTooltip","XLootScanTooltip",nil,UIParent)
				XLoot._scanTip:SetHyperlink(num)
				XLoot:Print("Caching item. Try again in a second...")
			end
		end
	end
end
